...
### In this comprised dataset by class ¿which was the most abundant family?
 #Family of() max (average which class = ?))
#    [max

MaxFamily <- function(x){ ## is a column
  ave<-AverageFamily$average[which(  AverageFamily$clase == x  )]
max_ave<-which(ave==max(ave))
max_fam<-AverageFamily$FO_Family[max_ave]
return(max_fam)
}

maximos<-lapply(unique(sort.by.family.CARD_MIBiG_FULL$clase),MaxFamily)
maximos
#which(a==max(a))
### In this comprised dataset by family ¿which was the most abundant class?
```
