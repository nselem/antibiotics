...
lactam database is composed by Biosynthetic gene clusters.  
...
### Homology searches between CARD and lactam  
Homology search was conducted using blast (e-vaule ) in an all vs all comparison with query antibiotic resistance genes from The Comprehensive Antibiotic Resistance Database (CARD) against genes in BGCs from lactam database.  
...
# BGC genuss in lactam  
The following figure shows the most populated sources in lactam DB.

```{r lactam, echo=FALSE,message=FALSE, include=FALSE, results='hide', warnings=FALSE}
lactam_file="data/lactam.meta"
lactam_table <- read_delim(lactam_file, "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
#Gen_Id, Familia (según fastOrtho)
#Cada renglón es una ocurrencia de un gen de la familia 1
colnames(lactam_table)=c("lactam_gen_Id","BGC_ID","genus","full_source")
BGCs<-length(unique(lactam_table$BGC_ID))

sortlactam<-lactam_table %>% group_by(genus) %>% mutate(n=n())%>% ungroup() %>% arrange(-n)
sortlactam$genus <- reorder(sortlactam$genus,sortlactam$genus,FUN=length)
levels(sortlactam$genus)
sort(table(sortlactam$genus))

#First_10_lactam<-sortlactam[1:784,]
#head(First_10_lactam)


lactam_genus<-ggplot(sortlactam, aes(genus))+ geom_histogram(stat="count", width = 0.7)+ theme(plot.title=element_text(size=20, 
                                    face="bold", 
                                    family="American Typewriter",
                                    color="tomato",
                                    lineheight=1.2),  # title
...
ggsave(file="results/lactam_genus.svg", plot=lactam_genus, width=30, height=15)

```
![lactam classes](lactam_classes.png)  

There are `r BGCs` different BGCs reported as lactam producers from antiSMASH-DB distributed in  several sources
...
```{r CAARD_families_in_lactam, echo=FALSE,message=FALSE, include=FALSE, results='hide', warnings=FALSE }
CARD_vs_lactam_file="data/CARDquery_LactamDB.blast1" #(Query card) vs lactam 
CARD_vs_lactam_table <- read_delim(CARD_vs_lactam_file, "\t", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
#Cada renglón es una ocurrencia de un gen de la familia 1
colnames(CARD_vs_lactam_table)=c("Gen_ID_CARD","CARD_SOURCE","lactam_gen_Id")
BGC_w_CARD<-length(unique(CARD_vs_lactam_table$BGC_ID)) # Number of times that BGC in lactam is hit of a Card gene

### Merging all tables in one
CARD_lactam_class<-merge(CARD_vs_lactam_table, lactam_table, by, by.x="lactam_gen_Id", by.y="lactam_gen_Id", sort = TRUE)
CARD_lactam_FULL<-merge(CARD_lactam_class, CARDFamiliesTable, by, by.x="Gen_ID_CARD", by.y="Gen_ID_CARD", sort = TRUE)
```

The following analysis finds the most represented CARD families by lactam classes. An average about how many hits normalized by the family size and the number of BGCs elements of the class.
...
!-->
```{r CAARD_families_in_lactam_2, echo=FALSE,message=FALSE, include=FALSE, results='hide', warnings=FALSE } 
## asign to each triada of family, Class, BGC element, how many hits were found
#sort.CARD_lactam_FULL<-CARD_lactam_FULL %>% group_by(FO_Family,genus,BGC_ID) %>%tally()
sort.pairs.F_C<-CARD_lactam_FULL %>% group_by(FO_Family,secondary_ID_CARD,genus,BGC_ID) %>%tally()

## Counts uniq triadas  by Family
#sort.by.family.CARD_lactam_FULL<-sort.CARD_lactam_FULL %>% group_by(FO_Family) %>% mutate(n=n())%>% ungroup() %>% arrange(-n)

sort.by.family.CARD_lactam_FULL<-sort.pairs.F_C %>% group_by(FO_Family,genus) %>% mutate(n=n())%>% ungroup() %>% arrange(-n)
# columna con numero de elementos de la familia que ocurre en ese renglon 
sort.by.family.CARD_lactam_FULL$FO_Family
...
sort.by.family.CARD_lactam_FULL$FamilyCardinality<-lapply(sort.by.family.CARD_lactam_FULL$FO_Family,FamilyNumber)

### Function to obtain cardinality of a class
ClassNumber <- function(x){ ## is a column
  elementos<-unique(sortlactam$n[which(sortlactam$genus ==x)])
return(elementos)
}
## Creating a column with family cardinality 
sort.by.family.CARD_lactam_FULL$genusCardinality<-lapply(sort.by.family.CARD_lactam_FULL$genus,ClassNumber)

## Obtaining the average 
sort.by.family.CARD_lactam_FULL$average=(sort.by.family.CARD_lactam_FULL$n)/((as.numeric(sort.by.family.CARD_lactam_FULL$FamilyCardinality))*(as.numeric(sort.by.family.CARD_lactam_FULL$genusCardinality)))
# number of families 
#length(unique(sort.by.family.CARD_lactam_FULL$FO_Family))
tail(sort.by.family.CARD_lactam_FULL$average)

AverageFamily<-sort.by.family.CARD_lactam_FULL %>% group_by(FO_Family,genus,average) %>%tally()

# First 16 families in CARD (Most abundant)
First_10sort.by.family.CARD_lactam_FULL<-sortCARD[1:1084,]
length(unique(First_10sort.by.family.CARD_lactam_FULL$FO_Family))
...
maximosbyClass<-lapply(unique(sort.by.family.CARD_lactam_FULL$genus),MaxFamilybyClass)
maximosbyClass
...
maximosbyFamily<-lapply(unique(sort.by.family.CARD_lactam_FULL$FO_Family),MaxClassbyFamily)
...
genussbyFamily<-lapply(unique(sort.by.family.CARD_lactam_FULL$FO_Family),FamilySpecificofClass)
...
The following figure shows lactam genuss, and in each class stacks a CARD family average
```{r CAARD_vs_lactam_family_average, echo=FALSE,message=FALSE, include=FALSE, results='hide', warnings=FALSE }
...
CARD_lactam_family_average<-ggplot(AverageFamily, aes(x = genus, y = average))+  geom_col(aes(fill = FO_Family), width = 0.7)+ theme(legend.position = "right", plot.title=element_text(size=20, 
                                    face="bold", 
                                    family="American Typewriter",
                                    color="tomato",
                                    lineheight=1.2),  # title
...
ggsave(file="results/CARD_lactam_family_average.svg", plot=CARD_lactam_family_average, width=30, height=15)
...
![CARD families in lactam classes](CARD_families_in_lactam)  
The following figure shows the average of CARD families by lactam classes



```{r familia_eje_X, eval=FALSE}
CARD_lactam_family_averageHorizontal<-ggplot(AverageFamily, aes(x = FO_Family , y = average))+  geom_col(aes(fill = genus), width = 0.7)+ theme(legend.position = "right", plot.title=element_text(size=20, 
                                    face="bold", 
                                    family="American Typewriter",
                                    color="tomato",
                                    lineheight=1.2),  # title
...
#ggsave(file="CARD_lactam_family_averageHorizontal.svg", plot=CARD_lactam_family_averageHorizontal, width=30, height=15)
ggsave(file="results/CARD_lactam_family_averageHorizontal_with_leyend.svg", plot=CARD_lactam_family_averageHorizontal, width=30, height=15)
...
CARD_lactam_family_averageHorizontalSubset<-  ggplot(bb_AverageFamily, aes(x = FO_Family , y = average))+  geom_col(aes(fill = genus), width = 0.7)+facet_grid(bb_AverageFamily$genus ~ .)+ theme(legend.position = "right", plot.title=element_text(size=50, 
                                    face="bold", 
                                    family="American Typewriter",
                                    color="tomato",
                                    lineheight=1.2),  # title
...
ggsave(file="CARD_lactam_family_averageHorizontal_with_leyend_Subset.svg", plot=CARD_lactam_family_averageHorizontalSubset, width=30, height=15)

```


```{r CAARD_vs_lactam, echo=FALSE,message=FALSE, include=FALSE, results='hide', warnings=FALSE }


## First of all, how many BGCs contain a hit in CARD 
## Count uniq BGCs

CARD_lactam_genuss<-ggplot(CARD_lactam_FULL,aes(genus)) + geom_bar(aes(fill=FO_Family), width = 0.5) + theme(legend.position = "none",plot.title=element_text(size=20, 
                                    face="bold", 
                                    family="American Typewriter",
                                    color="tomato",
                                    lineheight=1.2),  # title
            ...
            axis.text.x=element_text(size=10, 
                                     ...
                                     vjust=.5),  # X axis text
            axis.text.y=element_text(size=10))  # Y axis text
ggsave(file="CARD_lactam_genuss.png", plot=CARD_lactam_genuss, width=30, height=15)
...
From `r BGCs` different BGCs at lactam `r BGC_w_CARD` contains a CARD hit

The following figure shows which CARD families are the most  populated classes in lactam database.
and now an histogram by class with the number of BGCs with diferente frequecnies or CARd by color

![CARD families in lactam classes](CARD_lactam_genuss.png)  

which lactam classes are over represented as CARD hits
<!--
# Which lactam class has more CARD family hits
...
```{r CAARD_vs_lactam_2, echo=FALSE,message=FALSE, include=FALSE, results='hide', warnings=FALSE }
## Counting CARD FastOrthoFamilies per BGC
cuentas<-count(CARD_lactam_FULL %>% group_by(BGC_ID,FO_Family) %>% tally())
colnames(cuentas)=c("BGC_ID","cuentas")
head(cuentas)
## Here are in cuentas the number of FASTORTHO CARD families per BGC  
CARD_lactam_todo<-merge(CARD_lactam_FULL, cuentas, by, by.x="BGC_ID", by.y="BGC_ID", sort = TRUE)

##Now I wan thre columns class, BGC , # CARD families
Clas_BGC_Count<-CARD_lactam_todo %>% group_by(genus,BGC_ID,cuentas) %>% tally()
head (Clas_BGC_Count,100)
theme_set(theme_classic())


CARD_in_lactam<-ggplot(Clas_BGC_Count,aes(x = genus, y = cuentas))+ geom_boxplot() + theme(axis.text.x = element_text(angle=65, vjust=0.6)) + labs(title="Histogram on Categorical Variable", 
...
ggsave(file="CARD_in_lactam.png", plot=CARD_in_lactam, width=30, height=15)


```

![CARD families in lactam classes](CARD_in_lactam.png)  
aaaaaqui voy
